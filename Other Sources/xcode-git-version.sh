#!/bin/bash
# based on https://gist.github.com/karlvr/c93a98d7000ecb163895#file-xcode-git-version-sh
...
LATEST_TAG=$(git describe --tags --match '*.*' --abbrev=0 2>/dev/null) || LATEST_TAG="HEAD"
if [ $LATEST_TAG = "HEAD" ]
...
if [ $COMMIT_COUNT_SINCE_TAG = 0 ]; then
    ...
    # e.g. commit after 1.0 is 1.0.1, commit after 1.0.0 is 1.0.0.1
...
/usr/libexec/PlistBuddy -c "Add :CFBundleShortVersionString string $SHORT_VERSION" "$INFO_PLIST" 2>/dev/null || \
    /usr/libexec/PlistBuddy -c "Set :CFBundleShortVersionString $SHORT_VERSION" "$INFO_PLIST"

/usr/libexec/PlistBuddy -c "Add :CFBundleVersion string $BUNDLE_VERSION" "$INFO_PLIST" 2>/dev/null || \
